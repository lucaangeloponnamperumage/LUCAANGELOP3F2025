...
bool GestoreRAMOrdinata::inserisciOrdinato(int nuevoId, int nuovoTel) {
    ...
    while (i >= 0 && agenda[i].getId() > nuevoId) {
        agenda[i + 1] = agenda[i];
        i--;
    }
    
    agenda[i + 1].setId(nuevoId);
...
int GestoreMirroredRAM::ricercaBinaria(int idCercato) const {
    int inizio = 0;
    int fine = contatore - 1;
...
void GestoreMirroredRAM::caricaDaFile() {
    std::ifstream fileIn("agenda.txt");
    if (!fileIn) {
    ...
    while (fileIn >> readId >> readTel) {
        inserisciOrdinato(readId, readTel);
    }
    fileIn.close();
}

void GestoreMirroredRAM::salvaSuFile() {
    std::ofstream fileOut("agenda.txt");
    if (!fileOut) {
        return;
    }
    
    for (int i = 0; i < contatore; i++) {
        fileOut << agenda[i].getId() << " " << agenda[i].getTelefono() << "\n";
    }
    fileOut.close();
}

bool GestoreMirroredRAM::inserisciOrdinato(int nuovoId, int nuovoTel) {
    if (contatore >= MAX_CONTATTI) {
        return false;
    }
...
int GestoreMirroredRAM::cercaContatto(int idCercato) const {
    int indice = ricercaBinaria(idCercato);
    if (indice != -1) {
        return agenda[indice].getTelefono();
    }
...
bool GestoreMirroredRAM::eliminaContatto(int idDaEliminare) {
    int indice = ricercaBinaria(idDaEliminare);
...
void GestoreMirroredRAM::popolaAutomaticamente() {
    int idInit[] = {50, 20, 90, 10, 40, 80, 30, 70, 100, 60};
    int telInit[] = {5555, 5552, 5559, 5551, 5554, 5558, 5553, 5557, 5550, 5556};
...
void GestoreMirroredRAM::stampaAgenda() const {
    std::cout << "\n--- STATO ATTUALE AGENDA MIRRORED SU FILE (" << contatore << " contatti) ---\n";
    for(int i = 0; i < contatore; i++) {
        std::cout << "[" << i << "] ID: " << agenda[i].getId() << " -> Telefono: " << agenda[i].getTelefono() << "\n";
    }
    std::cout << "-------------------------\n";
